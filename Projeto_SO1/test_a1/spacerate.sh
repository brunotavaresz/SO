...
if [ $# -ne 2 ]; then # Se o número de argumentos for diferente de 2
  echo "Uso: $0 [-r] [-a] <file1> <file2>"
  exit 1
fi
...
compare_files() {
  ...
  while read -r size dir; do # Lê o ficheiro linha a linha
    dir_sizes1["$dir"]=$size # Adiciona o tamanho do diretório ao array
  done < "$file1" 
  ...
  for dir in "${!dir_sizes1[@]}"; do # Percorre os diretórios do primeiro ficheiro
    ...
    if [ -z "$size2" ]; then # Se o diretório não existe no segundo ficheiro
      ...
      size_diff=$((size2 - size1)) # Diferença entre os tamanhos
      echo "$size_diff $dir" # Imprime a diferença e o nome do diretório
    fi
  done
