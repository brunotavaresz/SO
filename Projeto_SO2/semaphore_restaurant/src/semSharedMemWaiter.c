...
static request waitForClientOrChef()
{
    ...
    sh->fSt.st.waiterStat = WAIT_FOR_REQUEST; // waiter esta a espera de um pedido
    ...
    // TODO insert your code here
    if (semDown (semgid, sh->waiterRequest) == -1) {  // decrementa o semaforo waiterRequest atraves do semDown para que o waiter receba o pedido do grupo ou do chef
    ...
    if (sh->fSt.foodOrder == FOOD_REQUEST) {
        req.reqType = FOODREQ; // pedido de comida
        saveState(nFic, &sh->fSt); // salva estado
    }
    else if (sh->fSt.foodOrder == FOODREADY) {
        req.reqType = FOODREADY; // comida pronta
        saveState(nFic, &sh->fSt); // salva estado
    }
...
static void informChef (int n)
{
    ...
    // TODO insert your code here
    sh -> fSt.st.waiterStat = INFORM_CHEF; // waiter esta a informar o chef
    sh->fSt.foodOrder = 1;
